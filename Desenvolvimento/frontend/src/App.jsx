...
import { cadastrarUsuario, loginUsuario, calcularSoh, cadastrarBateria, listarBateriasDoUsuario } from "./lib/api";

// ─── GLOBAL NAV ───────────────────────────────────────────────────────────────
// Fluxo: landing → register → onboarding → dashboard
let _currentPage = "landing";
let _usuarioAtual = JSON.parse(localStorage.getItem("usuarioEcoVolt") || "null");
let _ultimaAnalise = null;
const _subs = new Set();

function guardarUsuario(usuario) {
  _usuarioAtual = usuario;
  localStorage.setItem("usuarioEcoVolt", JSON.stringify(usuario));
}

function pegarUsuarioAtual() {
  return _usuarioAtual || JSON.parse(localStorage.getItem("usuarioEcoVolt") || "null");
}

function pegarIniciais(nome) {
  return (nome || "Usuário EcoVolt")
    .split(" ")
    .filter(Boolean)
    .slice(0, 2)
...
function guardarAnalise(resultado) {
  _ultimaAnalise = resultado;
}

function prepararBateriaParaTela(bateria) {
  const soh = Math.round(Number(bateria.soh || 0));
  const destino = soh >= 80 ? "Automotivo" : soh >= 60 ? "Segunda vida" : "Reciclagem";
  const data = bateria.criado_em ? new Date(bateria.criado_em).toLocaleDateString("pt-BR") : "-";

  return {
    id: `#B-${bateria.id}`,
    modelo: `${bateria.fabricante || ""} ${bateria.modelo || ""}`.trim(),
    soh,
    status: destino,
    result: destino,
    date: data,
...
const TopBar = ({ title }) => {
  ...
  const nomeUsuario = usuario?.nome || "Usuário EcoVolt";
  const iniciaisUsuario = pegarIniciais(nomeUsuario);
  return (
    <header style={{height:60,background:"#fff",borderBottom:"1px solid #F3F4F6",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 24px",flexShrink:0}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        ...
        <div onClick={()=>nav("profile")} style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",padding:"4px 8px",borderRadius:10}}>
          <div style={{width:32,height:32,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#00C896,#0A84FF)",color:"#fff",fontSize:11,fontWeight:900}}>{iniciaisUsuario}</div>
          <span style={{fontSize:13,fontWeight:600,color:"#374151"}}>{nomeUsuario}</span>
        </div>
      </div>
    </header>
  );
...
const RegisterScreen = () => {
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await cadastrarUsuario({ nome: `${nome} ${sobrenome}`.trim(), email, senha: pwd });
  ...
  const login = async () => {
    setLoading(true);
    try {
      const data = await loginUsuario({ email, senha: pwd });
      guardarUsuario(data.usuario);
      navigate("dashboard");
    } catch (err) {
      alert(err.message);
      ...
      <div style={{width:"100%",maxWidth:440,background:"#fff",borderRadius:20,border:"1px solid #F3F4F6",boxShadow:"0 1px 8px rgba(0,0,0,.04)",padding:32}}>
        ...
        <form onSubmit={submit} style={{display:"flex",flexDirection:"column",gap:13}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            {["Nome","Sobrenome"].map(l=>(
              <div key={l}>
                ...
                <input type="text" placeholder={l} required value={l==="Nome"?nome:sobrenome} onChange={e=>l==="Nome"?setNome(e.target.value):setSobrenome(e.target.value)} style={inp} onFocus={e=>e.target.style.borderColor=C.emerald} onBlur={e=>e.target.style.borderColor="#E5E7EB"}/>
              </div>
            ))}
          </div>
          {[["Email","email","seu@email.com",true],["Empresa","text","Nome da empresa (opcional)",false]].map(([l,t,p,r])=>(
            <div key={l}>
              <label style={{fontSize:12,fontWeight:700,color:"#4B5563",display:"block",marginBottom:5}}>{l}</label>
              <input type={t} placeholder={p} required={r} value={l==="Email"?email:undefined} onChange={e=>l==="Email"&&setEmail(e.target.value)} style={inp} onFocus={e=>e.target.style.borderColor=C.emerald} onBlur={e=>e.target.style.borderColor="#E5E7EB"}/>
            </div>
          ))}
          <div>
        ...
        <p style={{textAlign:"center",fontSize:13,color:"#9CA3AF",marginTop:14}}>
          Já tem conta?{" "}
          <button type="button" onClick={()=>nav("login")} style={{background:"none",border:"none",cursor:"pointer",color:C.emerald,fontWeight:800,fontSize:13}}>Entrar</button>
        </p>
      </div>
    </div>
  );
...
const LoginScreen = () => {
  ...
  const input = {width:"100%",padding:"10px 14px",borderRadius:10,border:"1px solid #E5E7EB",fontSize:14,background:"#F9FAFB",outline:"none",boxSizing:"border-box"};
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#F9FAFB",padding:24}}>
      <div style={{width:"100%",maxWidth:420,background:"#fff",borderRadius:20,border:"1px solid #F3F4F6",boxShadow:"0 1px 8px rgba(0,0,0,.04)",padding:32}}>
        <button onClick={()=>nav("landing")} style={{background:"none",border:"none",cursor:"pointer",color:"#9CA3AF",fontSize:13,marginBottom:22,padding:0}}>← Voltar</button>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:22}}>
          <div style={{width:34,height:34,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#00C896,#0A84FF)"}}>
            <Ic n="bolt" s={16} c="#fff"/>
          </div>
          <div><div style={{fontWeight:900,color:"#111827",fontSize:16}}>Entrar</div><div style={{fontSize:12,color:"#9CA3AF"}}>EcoVolt Analytics</div></div>
        </div>
        <form onSubmit={entrar} style={{display:"flex",flexDirection:"column",gap:13}}>
          <div>
            <label style={{fontSize:12,fontWeight:700,color:"#4B5563",display:"block",marginBottom:5}}>Email</label>
            <input type="email" placeholder="seu@email.com" required value={email} onChange={e=>setEmail(e.target.value)} style={input} onFocus={e=>e.target.style.borderColor=C.emerald} onBlur={e=>e.target.style.borderColor="#E5E7EB"}/>
          </div>
          <div>
            <label style={{fontSize:12,fontWeight:700,color:"#4B5563",display:"block",marginBottom:5}}>Senha</label>
            <input type="password" placeholder="••••••••" required value={senha} onChange={e=>setSenha(e.target.value)} style={input} onFocus={e=>e.target.style.borderColor=C.emerald} onBlur={e=>e.target.style.borderColor="#E5E7EB"}/>
          </div>
          <button type="submit" disabled={carregando} style={{padding:"13px 0",borderRadius:12,border:"none",background:carregando?"#9CA3AF":"linear-gradient(135deg,#00C896,#0A84FF)",color:"#fff",fontWeight:800,fontSize:14,cursor:carregando?"default":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
            {carregando?<><Spinner/>Entrando...</>:"Entrar"}
          </button>
        </form>
        <p style={{textAlign:"center",fontSize:13,color:"#9CA3AF",marginTop:14}}>
          Não tem conta?{" "}
          <button type="button" onClick={()=>nav("register")} style={{background:"none",border:"none",cursor:"pointer",color:C.emerald,fontWeight:800,fontSize:13}}>Criar conta</button>
        </p>
      </div>
    </div>
  );
    ...
    <div style={{minHeight:"100vh",overflowY:"auto",background:"#fff"}}>
      <nav style={{position:"sticky",top:0,zIndex:50,height:64,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 40px",background:"rgba(255,255,255,.96)",backdropFilter:"blur(12px)",borderBottom:"1px solid #F3F4F6"}}>
        ...
        <div style={{display:"flex",gap:10}}>
          <button onClick={()=>nav("login")} style={{padding:"8px 18px",borderRadius:10,border:"1px solid #E5E7EB",background:"transparent",color:"#374151",fontWeight:600,fontSize:13,cursor:"pointer"}}>Entrar</button>
      ...
      <div style={{minHeight:"88vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(160deg,#0d1117,#0a1628)",padding:"80px 40px",textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",maxWidth:660}}>
          ...
          <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
            ...
            <button onClick={()=>nav("login")} style={{padding:"13px 28px",borderRadius:12,border:"1px solid rgba(255,255,255,.2)",background:"transparent",color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer"}}>Já tenho conta</button>
          </div>
        </div>
      </div>
...
const DashboardScreen = () => {
  ...
  useEffect(() => {
    ...
    setCarregando(true);
    listarBateriasDoUsuario(usuario.id)
      .then(dados => setBaterias(dados.map(prepararBateriaParaTela)))
      .catch(erro => alert(erro.message))
      .finally(() => setCarregando(false));
  }, [usuario?.id]);

  const totalBaterias = baterias.length;
  const sohMedio = totalBaterias
    ? Math.round(baterias.reduce((soma, bateria) => soma + bateria.soh, 0) / totalBaterias)
    : 0;
  const bateriasCriticas = baterias.filter(bateria => bateria.soh < 60).length;
  const bateriasSegundaVida = baterias.filter(bateria => bateria.soh >= 60 && bateria.soh < 80).length;

  const metricas = [
    {icon:"trending",label:"SOH Médio",value:`${sohMedio}%`,color:C.emerald,sparkData:[sohMedio,sohMedio,sohMedio,sohMedio]},
    {icon:"battery",label:"Baterias Cadastradas",value:String(totalBaterias),color:C.azure,sparkData:[0,totalBaterias,totalBaterias,totalBaterias]},
    {icon:"recycle",label:"Para Segunda Vida",value:String(bateriasSegundaVida),color:C.purple,sparkData:[0,bateriasSegundaVida,bateriasSegundaVida,bateriasSegundaVida]},
    {icon:"bell",label:"Baterias Críticas",value:String(bateriasCriticas),color:C.danger,sparkData:[0,bateriasCriticas,bateriasCriticas,bateriasCriticas]},
  ];

  const corStatus = {"Automotivo":C.emerald,"Segunda vida":C.azure,"Reciclagem":C.danger};
      ...
      <div style={{display:"flex",flexDirection:"column",gap:18}}>
        ...
        <div style={{background:"#fff",borderRadius:16,border:"1px solid #F3F4F6",overflow:"hidden"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 18px",borderBottom:"1px solid #F9FAFB"}}>
            <h3 style={{fontWeight:800,color:"#111827",margin:0,fontSize:14}}>Baterias Cadastradas</h3>
          ...
          {carregando&&<div style={{padding:18,fontSize:13,color:"#9CA3AF"}}>Carregando baterias...</div>}
          {!carregando&&baterias.length===0&&(
            <div style={{padding:22,fontSize:13,color:"#6B7280"}}>Nenhuma bateria cadastrada ainda. Clique em cadastrar para fazer a primeira análise.</div>
          )}
          {!carregando&&baterias.length>0&&(
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr style={{background:"#F9FAFB"}}>
                {["ID","Modelo","SOH","Classificação","Data"].map(h=><th key={h} style={{padding:"9px 14px",textAlign:"left",fontSize:11,fontWeight:700,color:"#9CA3AF"}}>{h}</th>)}
              </tr></thead>
              <tbody>
                {baterias.map(bateria=>(
                  <tr key={bateria.id} onClick={()=>nav("analysis")} style={{borderTop:"1px solid #F9FAFB",cursor:"pointer"}}
                    onMouseEnter={e=>e.currentTarget.style.background="#F9FAFB"}
                    onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                    <td style={{padding:"11px 14px",fontFamily:"monospace",fontSize:11,color:"#9CA3AF"}}>{bateria.id}</td>
                    <td style={{padding:"11px 14px",fontSize:13,fontWeight:600,color:"#1F2937"}}>{bateria.modelo}</td>
                    <td style={{padding:"11px 14px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:7}}>
                        <div style={{width:52,height:4,borderRadius:99,background:"#F3F4F6",overflow:"hidden"}}><div style={{height:"100%",width:`${bateria.soh}%`,background:bateria.soh>=80?C.emerald:bateria.soh>=60?C.warning:C.danger,borderRadius:99}}/></div>
                        <span style={{fontSize:12,fontWeight:700,color:bateria.soh>=80?C.emerald:bateria.soh>=60?C.warning:C.danger}}>{bateria.soh}%</span>
                      </div>
                    </td>
                    <td style={{padding:"11px 14px"}}><span style={{fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:99,background:`${corStatus[bateria.status]}15`,color:corStatus[bateria.status]}}>{bateria.status}</span></td>
                    <td style={{padding:"11px 14px",fontSize:11,color:"#D1D5DB"}}>{bateria.date}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const BatteryRegisterScreen = () => {
  const [,nav] = usePage();
  const [loading,setLoading] = useState(false);
  const [battery,setBattery] = useState({ fabricante:"", modelo:"", ano:"", cap_original:"", cap_atual:"", ciclos:"", temperatura:"", tensao:"" });
  const updateBattery = (key, value) => setBattery(prev => ({ ...prev, [key]: value }));
  const submit = async (e) => {
    e.preventDefault();
    setLoading(true);
    try {
      const payload = {
        fabricante: battery.fabricante,
        modelo: battery.modelo,
        ano: Number(battery.ano),
        cap_original: Number(battery.cap_original),
        cap_atual: Number(battery.cap_atual),
        ciclos: Number(battery.ciclos),
      };
      const result = await calcularSoh(payload);
      if (_usuarioAtual?.id) {
        try { await cadastrarBateria({ usuario_id: _usuarioAtual.id, ...payload }); } catch (_) {}
      }
      guardarAnalise(result);
      nav("analysis");
    } catch (err) {
      alert(err.message);
    } finally {
      ...
      <div style={{maxWidth:620,margin:"0 auto"}}>
        <div style={{background:"#fff",borderRadius:18,border:"1px solid #F3F4F6"}}>
          ...
          <form onSubmit={submit} style={{padding:22,display:"flex",flexDirection:"column",gap:14}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {[
                {l:"Fabricante",k:"fabricante",t:"select",opts:["BYD","CATL","LG Chem","Panasonic","Samsung SDI","Tesla","Outro"]},
                {l:"Modelo",k:"modelo",t:"text",p:"Ex: Blade 100kWh"},
                {l:"Ano de Fabricação",k:"ano",t:"number",p:"Ex: 2021"},
                {l:"Capacidade Original (kWh)",k:"cap_original",t:"number",p:"Ex: 100"},
                {l:"Capacidade Atual (kWh)",k:"cap_atual",t:"number",p:"Ex: 84"},
                {l:"Ciclos de Carga",k:"ciclos",t:"number",p:"Ex: 1240"},
                {l:"Temperatura Op. (°C)",k:"temperatura",t:"number",p:"Ex: 35"},
                {l:"Tensão Máxima (V)",k:"tensao",t:"number",p:"Ex: 400"},
              ].map(f=>(
                <div key={f.l}>
                  <label style={{fontSize:12,fontWeight:700,color:"#4B5563",display:"block",marginBottom:5}}>{f.l}</label>
                  {f.t==="select"
                    ?<select required value={battery[f.k]} onChange={e=>updateBattery(f.k,e.target.value)} style={{...inp,appearance:"none"}} onFocus={e=>e.target.style.borderColor=C.emerald} onBlur={e=>e.target.style.borderColor="#E5E7EB"}><option value="">Selecionar...</option>{f.opts.map(o=><option key={o}>{o}</option>)}</select>
                    :<input type={f.t} placeholder={f.p} required={["modelo","ano","cap_original","cap_atual","ciclos"].includes(f.k)} value={battery[f.k]} onChange={e=>updateBattery(f.k,e.target.value)} style={inp} onFocus={e=>e.target.style.borderColor=C.emerald} onBlur={e=>e.target.style.borderColor="#E5E7EB"}/>}
                </div>
              ))}
            </div>
            <div>
...
const AnalysisScreen = () => {
  const [,nav] = usePage();
  const analise = _ultimaAnalise || { fabricante:"CATL", modelo:"LFP 75kWh", ano:2020, ciclos:1240, cap_atual:55.5, soh:74, classificacao:"Segunda vida", recomendacao:"Desgaste moderado, com potencial para reaproveitamento energetico em sistemas estacionarios. Recomendamos cotacao no marketplace de segunda vida." };
  const soh=Math.round(Number(analise.soh || 0)),col=soh>=80?C.emerald:soh>=60?C.warning:C.danger,cls=analise.classificacao || (soh>=80?"Automotivo":soh>=60?"Segunda vida":"Reciclagem");
  const hist=[100,97,95,92,89,86,82,78,soh],proj=[soh,Math.max(0,soh-4),Math.max(0,soh-8),Math.max(0,soh-12),Math.max(0,soh-16),Math.max(0,soh-20),Math.max(0,soh-24)];
      ...
      <div style={{maxWidth:720,margin:"0 auto",display:"flex",flexDirection:"column",gap:14}}>
        <div style={{background:"#fff",borderRadius:18,border:"1px solid #F3F4F6",padding:22,display:"flex",flexWrap:"wrap",alignItems:"center",gap:20}}>
          ...
          <div style={{flex:1}}>
            ...
            <h2 style={{fontWeight:900,fontSize:18,color:"#111827",margin:"0 0 4px"}}>{analise.fabricante} {analise.modelo} · {analise.ano}</h2>
            <p style={{fontSize:12,color:"#9CA3AF",margin:"0 0 10px"}}>{analise.ciclos} ciclos · Capacidade atual: {analise.cap_atual} kWh</p>
    ...
    <Shell title="Notificações">
      <div style={{maxWidth:580,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:9}}>
          {notes.length===0&&<div style={{padding:14,fontSize:13,color:"#6B7280",background:"#fff",borderRadius:13,border:"1px solid #F3F4F6"}}>Nenhuma notificação cadastrada no backend.</div>}
          {notes.map(n=>(
...
const ProfileScreen = () => {
  ...
  const usuario = pegarUsuarioAtual();
  const nomeUsuario = usuario?.nome || "Usuário EcoVolt";
  const emailUsuario = usuario?.email || "email@ecovolt.com";
  const iniciaisUsuario = pegarIniciais(nomeUsuario);
  ...
  useEffect(() => {
    ...
    listarBateriasDoUsuario(usuario.id)
      .then(dados => setAnalises(dados.map(prepararBateriaParaTela)))
      .catch(erro => alert(erro.message));
  }, [usuario?.id]);
  const totalAnalises = analises.length;
  const sohMedioPerfil = totalAnalises ? Math.round(analises.reduce((soma, bateria) => soma + bateria.soh, 0) / totalAnalises) : 0;
  const bateriasCriticasPerfil = analises.filter(bateria => bateria.soh < 60).length;
  const corResultado={"Automotivo":C.emerald,"Segunda vida":C.azure,"Reciclagem":C.danger};
      ...
      <div style={{maxWidth:680,margin:"0 auto",display:"flex",flexDirection:"column",gap:14}}>
        <div style={{background:"#fff",borderRadius:18,border:"1px solid #F3F4F6",padding:22}}>
          <div style={{display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
            <div style={{width:56,height:56,borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#00C896,#0A84FF)",color:"#fff",fontSize:18,fontWeight:900}}>{iniciaisUsuario}</div>
            <div style={{flex:1}}>
              <div style={{fontSize:19,fontWeight:900,color:"#111827"}}>{nomeUsuario}</div>
              <div style={{fontSize:13,color:"#9CA3AF"}}>{emailUsuario}</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:20,textAlign:"center"}}>
              {[[String(totalAnalises),"Análises"],[`${sohMedioPerfil}%`,"SOH médio"],[String(bateriasCriticasPerfil),"Críticas"]].map(([v,l])=>(
        ...
        <div style={{background:"#fff",borderRadius:18,border:"1px solid #F3F4F6",overflow:"hidden"}}>
          <div style={{display:"flex",borderBottom:"1px solid #F3F4F6"}}>
            {[{id:"analises",l:"Análises"},{id:"financeiro",l:"Financeiro"},{id:"ambiental",l:"Ambiental"},{id:"configuracoes",l:"Configurações"}].map(t=>(
              <button key={t.id} onClick={()=>setAba(t.id)} style={{flex:1,padding:"13px 0",fontSize:13,fontWeight:700,border:"none",background:"transparent",cursor:"pointer",color:aba===t.id?C.emerald:"#9CA3AF",borderBottom:`2px solid ${aba===t.id?C.emerald:"transparent"}`}}>{t.l}</button>
            ))}
          </div>
          <div style={{padding:18}}>
            {aba==="analises"&&(
              <div style={{display:"flex",flexDirection:"column",gap:3}}>
                {analises.length===0&&<div style={{padding:"9px 10px",fontSize:13,color:"#6B7280"}}>Nenhuma bateria cadastrada ainda.</div>}
                {analises.map(a=>(
                  <div key={a.id} onClick={()=>nav("analysis")} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 10px",borderRadius:9,cursor:"pointer"}}
                    ...
                    <span style={{fontSize:10,padding:"2px 8px",borderRadius:99,fontWeight:700,background:`${corResultado[a.result]}15`,color:corResultado[a.result]}}>{a.result}</span>
            ...
            {aba==="financeiro"&&(
              <div style={{padding:14,fontSize:13,color:"#6B7280",lineHeight:1.6}}>
                Dados financeiros ainda não foram cadastrados no backend.
              </div>
            )}
            {aba==="ambiental"&&(
              <div style={{padding:14,fontSize:13,color:"#6B7280",lineHeight:1.6}}>
                Dados ambientais ainda não foram cadastrados no backend.
              </div>
            )}
            {aba==="configuracoes"&&(
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {[["Nome completo","text",nomeUsuario],["Email","email",emailUsuario]].map(([l,t,v])=>(
                  <div key={l}>
                    <label style={{fontSize:12,fontWeight:700,color:"#4B5563",display:"block",marginBottom:5}}>{l}</label>
